classdef SHHMMshared < handle
    ...
    methods
        function self = SHHMMshared(NC,dim,D,obsTypes,alpha_0,Aalpha_0,pi0alpha_0)
            ...
            self.obsModels=dists.GPEF(obsTypes);
        ...
        function L = update(self,data)
            ...
            ptemp = zeros(size(self.HMMs{1}.p));
            for i=1:self.NC
                self.HMMs{i}.updateMarkovparms(data,self.p(i,:)); 
                ptemp=ptemp+p(i,:).*self.HMMs{i}.p
            end
            self.obsModel.update(data,ptemp);
            for i=1:self.NC
        ...
        function res = KLqprior(self)
            res = + self.pi.KLqprior-(NC-1)/NC*self.obsModels.KLqprior;
            for i=1:self.NC
