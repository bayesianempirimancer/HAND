classdef mixHHMMupdated < handle
    properties
    ...
    methods
        ...
        function self = mixHHMMupdated(NC,Qdim,dim,D, obsTypes, alpha_0, Qalpha_0, Qpi0alpha_0, Aalpha_0, pi0alpha_0,NDpts)
            if(isempty(obsTypes))
                obsTypes{1}.dist='mvn';
                obsTypes{1}.idx=[1:D];
                self.D = D;
            ...
            if(exist('NDpts','var'))
                self.NDpts=NDpts;
                for i=1:Ndatapoints
                    self.single_HHMMs{i}=HHMM(Qdim,dim,D,obsTypes,Qalpha_0,Qpi0alpha_0,Aalpha_0,pi0alpha_0);
                end
            else
                self.NDpts=0;
                self.single_HHMMs=[];
...
%                 return
        ...
        function split(self,data,modeliters,idx1)
            ...
            [m,idx2]=sort(self.NA);
            if(m(1)>1/2) 
               fprintf('split: no empty clusters\n')
               return
            end
            idx2=idx2(1);
            ...
            elseif(idx1==idx2)
                fprintf('cant split cluster with no elements\n')
                return
            end
            ...
            if(length(pidx)<2)
                fprintf('cant split cluster with one element\n')
                return
            else
                fprintf(['splitting cluster ', num2str(idx1),'\n'])
            end
