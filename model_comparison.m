...
% and model comparison between vanilla LDS and LDS w/ inputs and bias
...
model_inputs = VBLDSinputs(k, u, d, h);    
model_vanilla = VBLDS(k,d);

iters = 50;
model_inputs.fit(U,Y,iters)
model_vanilla.fit(Y,iters);


%% model diagnostics

model_inputs.plotLs 
model_vanilla.plotLs 

% model_inputs.negdLs

model_inputs.r(Xtrue)
model_vanilla.r(Xtrue)

model_inputs.R2
model_vanilla.R2

model_inputs.A.mean 
model_vanilla.A.mean 

model_inputs.Ctrue
model_vanilla.Ctrue

inv(model_inputs.A.invU.mean)
inv(model_vanilla.A.invU.mean)

inv(model_inputs.C.invU.mean) 
inv(model_vanilla.C.invU.mean)
